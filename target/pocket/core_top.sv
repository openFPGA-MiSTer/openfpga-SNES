...
module core_top (
  ...
  parameter SAVE_STATES = 1'b0;

  // Bridge address region (upper nibble) that carries the save state blob
  ...
  // Save states: the blob is staged in cart SDRAM and moved over the bridge
  // region SS_REGION (0x4xxxxxxx); see save_state_controller.sv /
  // save_state_mem.sv. The engine emits WRAM+VRAM+ARAM (0x40000) plus
  // register sections and DSPn state (~0x18xx), and boot1 also dumps cart
  // RAM (1KB << RAMSZ, up to 128KB) into the blob, so the declared size
  // must follow the cart: anything short of the real payload truncates the
  // transfer and comes back as corruption on the next cold load. The worst
  // case (SS_BLOB_FIXED + 128KB = 0x64000) must also stay within the 1MB
  // that save_state_mem.sv leaves above SSBASE, or the blob wraps past the
  // end of SDRAM
  localparam [31:0] SS_BLOB_FIXED = 32'h44000;

  wire [31:0] ss_blob_size = SS_BLOB_FIXED + (ram_size != 0 ? (32'd1024 << ram_size) : 32'd0);

  wire savestate_supported = SAVE_STATES;
  ...
  reg [7:0] rom_type;
  reg [3:0] rom_size;
  // ram_size also sizes the savestate blob; init so the declared size is
  // sane even before the loader has sent the cart parameters
  reg [3:0] ram_size = 0;
  reg PAL;
  ...
  generate
    if (SAVE_STATES == 1'b1) begin : ss_ctrl
      // Blob upload: APF bridge writes into SDRAM staging
      data_loader #(
      ...
      // Blob readback: APF bridge reads out of SDRAM
      data_unloader #(
          .ADDRESS_MASK_UPPER_4(SS_REGION),
          .ADDRESS_SIZE(20),
    ...
    end else begin : ss_ctrl_off
      assign ss_read_data = 32'd0;
      assign ss_save = 1'b0;
      assign ss_load = 1'b0;
      assign ss_abort = 1'b0;
      assign ss_force_nmi = 1'b0;
      assign ss_pause = 1'b0;
